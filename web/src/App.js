...
import { listRateCards, listRooms } from './api/rooms'
...
class App extends Component {
  ...
  setRoom = id => {
    const ratecard = this.state.ratecardsData.find(ratecard => ratecard._id === id)
    this.setState({currentRate : ratecard })
  }
  ...
  loadRateCards = () => {
    ...
    this.state.roomData.forEach(room => {
      ...
      room.bookings.forEach(booking => {
        if (booking.user === userId) {
          // Push all bookings where the current userId is equal to the booking's userId into myRateCards
          booking.roomId = room._id
          myRateCards.push(booking)
        }
      })
    })
    this.setState({ userBookings: myRateCards })
  }
  ...
  render() {
    ...
    const loadRateCards = this.loadRateCards
        ...
        <div id="app" className="App">
          <Fragment>
              <Switch>
                ...
                <Route path="/ratecards" exact render={requireAuth(() => (
                    <Fragment>
                      {!!decodedToken &&
                        !!ratecardsData && (
                          <div className="wrapper">
                            <div className="header header__nav header--flex">
                              ...
                              <NavBar
                                signOut={signOut}
                                loadRateCards={loadRateCards}
                                user={signedIn ? decodedToken.sub : null}
                              />
                            </div>
                            <div className="wrapper__content--bookings">
                              <div className="header__page">
                                <h2 className="header__heading header__heading--sub">Rate Cards</h2>
                              </div>
    ...
    if (signedIn) {
      ...
      setTimeout(() => {
        ...
        .then(ratecards => {
          ...
          this.setState({ ratecardsData: ratecards })
          // load the current user's bookings
          this.loadRateCards()
          // the state's current room defaults to first room
          const ratecard = this.state.ratecardsData[0]
          this.setRoom(ratecard._id)
